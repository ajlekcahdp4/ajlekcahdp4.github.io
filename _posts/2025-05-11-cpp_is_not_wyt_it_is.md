...
a while I was living in the "C++ bubble" as one could say.  Primarily speaking to other people
learning and doing C++ daily. But lately I started speaking more and more to non-C++
...
abstraction while remaining backwards compatibility with C. The possibility of writing classes
...
Since 2011 C++'s speed of evolution drastically increased and the way C++ program
looks and feels started to change. Since 2011 C++ standard committee started a
regular 3-year release process. Which means that every 3 years user were getting newer useful
...
has with C++ is that it is "easy" to get memory vulnerabilities such as memory leak, uninitialized
variable read/write. However I argue that this relates to the old C++ and much less to the
...
Returning raw pointer has obvious downsides. For the user of this function it is unclear,
who owns this object, should he call free later and whether he can get a `NULL` pointer from it. If he should free the object he might forget to do so. The problem here arises from pointers being too generic types. It can be used for every type of storage and ownership. C++ solves this problem since C++11 standard by introducing smart pointers. In an unlikely case you never heard of them I'll give two examples. Let us again consider our factory function. If one were to write an API that gives exclusive ownership of widgets to the caller it would have a signature containing `std::unique_ptr`:

```cpp
std::unique_ptr<Widget> factory();
```

Now user can clearly see that this function returns pointer with exclusive ownership (can be destroyed when not needed) and he doesn't have to worry much about its lifetime as it will live as long as `unique_ptr` exists. Now he also doesn't have to worry about deallocating memory as pointer will take care of it.
...
And from this signature users will see that their ownership of the widget is not exclusive, so they shouldn't destroy the object when not needed. Users still have no worries about lifetime as widget will live at least as long as their `shared_ptr` does. And, as a bonus, users still won't worry about memory deallocation as `shared_ptr` will take care of that when both client and `Application` class won't need it anymore.
...
anyone who has somewhat decent C++ experience will understand them and won't make bugs. Of
course 95% is not enough. And there we can talk about an excellent set of tools that check
lifetime-based UB for you in very efficient way. Tools like valgrind and sanitizers
...
Here `create_a` function returns a reference to a local variable, which is destroyed during return so this reference is dangling and the proglem contains undefined behaviour. However in such a trivial UB case every compiler I [tried](https://godbolt.org/z/9G7j5MnnE) (Including gcc, clang and MSVC) caught this error and produced a nice warning. But let's say you are the weird person who does not use `-Werror` flag to treat warnings as errors and doesn't even read his warnings, redirecting all of the compiler output to `/dev/null`. By executing this program you can get any result printed on the screen or none at all, get a Segmentation fault or no error at all. With clang I got zero printed on the screen and everything seemed fine:

```sh
❯ ./main
0
```
Let's now use some of the tools available for C++ developers to debug and diagnose the problem. First we will use sanitizer, which is a library you link with to diagnose memory access issues, UB or multithread issues. Sanitizers are a great tools that you should ideally use in your regular builds (perhaps nightly or weekly) to catch problems in the first place, other than use them for debugging. First, we recompile our executable with address and UB sanitizers. Then, after launching it we will get a crash:
...
highly reduced probavility of writing out-of-bounds access or dereferencing past the end iterator
...
This works ([as far as I know](https://godbolt.org/z/EvKorG64f)). But the code is a mess. It is becomes hard to reason about iterators even in such small example. I myself in fact got an out-og-bounds access the first time I wrote it. Let's not imagine more complex cases (such as iterating over each 5 or more elements). Let's now take a look at how you might do it with C++23 ranges:
...
int main() {
    auto r = factory(1, 1);
    assert(r.has_value());
    add_widget_to_app(std::move(r.value()));
    r = factory(-1, 1);
    if (!r.has_value())
        std::cerr << "error: " << r.error() << '\n';
    else
        ass_widget_to_app(std::move(r.value()));
}
```

This example [works great](https://godbolt.org/z/PGo99xY6K), code looks clean and can be used in environments without exceptions or anywhere you see fit.
